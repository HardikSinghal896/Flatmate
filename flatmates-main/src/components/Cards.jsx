  ...
  useEffect(() => {
    ...
    if (paramT === null) {
      setArr(cardArray.filter((item) => {
        return (item.type =="rooms" || item.type === "room_mates" || item.type === "pg")
      }))
    }
    else if (paramT == "nhp") {
      setArr(cardArray.filter((item) => {
    ...
    <div className={style.mainContainer}>
      {
        arr.map((item) => {
          return (
            <Card gender={item.gender} id={item.id} sharing={item.sharing} rent={item.rent} passimg={item.filename} location={item.location}  name={item.name}/>
          )
        })
      }
    </div>
...
const Card = ({ id,sharing, location,name,rent,passimg,gender}) => {
  ...
  return (
    <>
      ...
      <div className={style.container} onClick={handleCardClick}>
        <div>
          <img src={passimg && passimg[0]} alt="" width={"100px"} height={"100px"} />
        </div>
        <div>
          <h3>{name}</h3>
          <p>{location}</p>
